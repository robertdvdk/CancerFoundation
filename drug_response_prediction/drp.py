...
def MetadataGenerate(
    ...
    drug_match_list = [
        item
        for item in experiment_data.index
    ...
    for each_drug in experiment_data_filtered.index:
        for each_cellline in experiment_data_filtered.columns:
            ...
            if (
                str(pubchem_id) in drug_pubchem_id_set
                and each_cellline in gexpr_feature.index
            ):
                if (
    ...
    print(
        "%d instances across %d cell lines and %d drugs were generated."
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default="0",
        help="GPU devices. Use -1 for CPU.",
    )
    parser.add_argument(
        "--embedding_path",
        type=str,
    ...
    parser.add_argument("--test_drug", type=str, default=None, help="Hold-out drug.")
    parser.add_argument(
        "--val_drug", type=str, default=None, help="Drug used for early stopping."
    )
    parser.add_argument(
        "--run_id",
        type=str,
        required=True,
        help='Unique ID for the script run, e.g., "1", "2", to ensure unique output files.',
    )
    ...
    assert (args.test_drug is None and args.val_drug is None) or (
        args.test_drug != args.val_drug
    ), "test_drug and val_drug must either both be None, or refer to different drugs."
    ...
    if args.test_drug is None and args.val_drug is None:
        ...
        data_train_idx, data_test_idx = DrugSplit(data_train_idx, args.val_drug)
    ...
    train_loader = DataLoader(
        list(zip(X_drug_data_train, X_gexpr_data_train, Y_train)),
    ...
    test_loader = DataLoader(
        list(zip(X_drug_data_test, X_gexpr_data_test, Y_test)),
        batch_size=batch_size,
        shuffle=False,
        num_workers=8,
    )
    ...
    real_loader = DataLoader(
        list(zip(X_drug_data_real, X_gexpr_data_real, Y_real)),
        batch_size=batch_size,
        shuffle=False,
        num_workers=8,
    )

    drug_gcnn = UGCNN(input_dim=75, hidden_dims=[256, 256, 256], out_channels=100)
    ...
    parameters = (
        list(drug_gcnn.parameters())
        + list(gexpr_mlp.parameters())
        + list(comb_mlp.parameters())
    )
    ...
    for epoch in range(epochs):
        ...
        for i, (drug, gexpr, target) in enumerate(progress_bar):
            target, drug, gexpr = place_on_device(args.gpu_id, target, drug, gexpr)
            optimizer.zero_grad()
            drug = drug_gcnn(drug)
        ...
        print(f"Epoch {epoch+1}, Validation Loss: {val_loss:.4f}, PCC: {pcc:.4f}")
    ...
    pcc_real = compute_pcc(np.concatenate(all_targets), np.concatenate(all_outputs))
    print(f"PCC on hold-out data: {pcc_real}.")

    embedding_identifier = args.embedding_path[
        args.embedding_path.rfind("/") + 1 : args.embedding_path.rfind(".csv")
    ]

    os.makedirs("./results", exist_ok=True)

    # --- Start of new code block ---
    # Save the PCC score to a separate file in append mode
    if args.test_drug is None:
        embedding_identifier += "_general"

    pcc_results_path = f"./results/PCC_{embedding_identifier}_{args.run_id}.csv"
    file_exists = os.path.isfile(pcc_results_path)

    with open(pcc_results_path, "a", newline="") as csvfile:
        fieldnames = ["test_drug", "val_drug", "PCC"]
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)

        if not file_exists:
            writer.writeheader()  # Write header only if the file is new

        # Use 'N/A' if drugs are not specified (cell line split case)
        test_drug_id = args.test_drug if args.test_drug is not None else "N/A"
        val_drug_id = args.val_drug if args.val_drug is not None else "N/A"

        writer.writerow(
            {"test_drug": test_drug_id, "val_drug": val_drug_id, "PCC": pcc_real}
        )

    print(f"PCC score appended to {pcc_results_path}")
    # --- End of new code block ---

    task = (
        "cell_line"
        if (args.val_drug is None and args.test_drug is None)
        else f"drug_{args.test_drug}"
    )
    ...
    df.to_csv(
        f"./results/hold_out_{task}_{embedding_identifier}_{args.run_id}.csv",
        index=False,
    )
    print(
        f"Results saved to ./results/hold_out_{task}_{embedding_identifier}_{args.run_id}.csv"
    )
